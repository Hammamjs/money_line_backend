...
export const ordersService = {
  create: async ({
    ...
    try {
      ...
      const order = await ordersRepository.create({
        ...
        fromCurrencyId: fromAssetId,
        toCurrencyId: toAssetId,
        note,
        amount,
        transactorId,
      });
  ...
  updateStatus: async (
    ...
    if (status === undefined) throw Errors.badRequest('Nothing to update');

    const order = await ordersRepository.updateStatus(id, transactorId, {
      status,
    });
  ...
  getAll: async ({
    ...
    return ordersRepository.getAll({ status, transactorId });
  },
